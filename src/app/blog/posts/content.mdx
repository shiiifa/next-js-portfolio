...
Replace the content in the `resources/content.js` file with your own content. You can use custom components in most cases, since the props are usually declared as `ReactNode` instead of `string`, but it is important that you need to import them in the file.
...
`const person = {
    ...
    languages: ["English", "Bahasa"], // optional: Leave the array empty if you do not want to display languages
};`,
    language: "tsx",
    label: "src/app/resources/content.js"
  }
...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but do not forget to import them in `src/once-ui/icons.ts`. Read the Once UI documentation](https://once-ui.com/docs/icons) for more information.
...
Each page have its own object that manages the personalized content, such as headings, subheadings, images and dynamic data.
...
`const home = {
    ...
    subline: (
      <>
        I am Selene, a design engineer at <InlineCode>FLY</InlineCode>, where I craft intuitive
